...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    for idx, agent in enumerate(cpu_agents):
        ...
        round_timeouts = counts[0]
        total_timeouts += round_timeouts
        round_forfeits = counts[1]
        total_forfeits += round_forfeits
        ...
        time_end = time.time()
        print(" -- {:4.0f} timeouts {:4.0f} fortfeits {:5.1f} mins elapsed".format(round_timeouts, round_forfeits,
